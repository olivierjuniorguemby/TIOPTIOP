...
async function main() {
    ...
    console.log("============================================================");
    console.log(" TIOPTIOP — 13.9.5.4 — AUDIT FINAL PAIEMENTS");
    ...
    */
    ...
    section("RESULTAT 13.9.5.4");
    console.log(`Erreurs critiques : ${errors}`);
    console.log(`Avertissements    : ${warnings}`);

    if (errors > 0) {
        console.log("❌ 13.9.5.4 NON VALIDEE : corriger les anomalies critiques.");
        process.exitCode = 1;
        return;
    }

    console.log("✅ 13.9.5.4 : intégrité financière critique validée.");
    if (warnings > 0) {
...
main()
    .catch(error => {
        console.error("❌ Audit final 13.9.5.4 :", error);
        process.exitCode = 1;
    })
    .finally(async () => {
